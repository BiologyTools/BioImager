...
namespace BioImager
{
    ...
    {
        GLWindow window;
        SlideGLArea gl = new SlideGLArea();
        SlideRenderer slideRenderer;
        private OverviewOverlayPanel overviewOverlay;
        ///Initializing the image viewer. */
        public ImageView(BioImage im, bool OpenWithOpenSlide = true)
        {
            string file = im.ID.Replace("\\", "/");
            InitializeComponent();
            bool useGpuTiles = im.Resolutions.Count > 1;
            BioLib.SlideSourceBase.UseGPU = useGpuTiles;
            OpenSlideGTK.SlideSourceBase.useGPU = useGpuTiles;
            viewpanel.Paint += Viewpanel_Paint;
            AttachViewerInput(viewpanel);

            if (im.isPyramidal)
            {
                gl.Size = new System.Drawing.Size(800, 600);
            ...
            else
            {
                ...
                gl.Width += 18;
                gl.Height += 18;
                Tools.selectBoxSize = GetSelectBoxSize();
            }
            App.viewer = this;
            serie = im.series;
            ...
            this.KeyPress += ImageView_KeyPress;
            this.KeyUp += ImageView_KeyUp;
            TimeFps = 60;
            ZFps = 60;
            CFps = 1;
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                case WM_KEYDOWN:
                    ImageView_KeyDown(this, new KeyEventArgs(keyData));
                    break;

                case WM_KEYUP:
                    ImageView_KeyUp(this, new KeyEventArgs(keyData));
                    break;

                case WM_CHAR:
                    char ch = (char)msg.WParam;
                    ImageView_KeyPress(this, new System.Windows.Forms.KeyPressEventArgs(ch));
                    break;
            }
        ...
        private void ImageView_SizeChanged(object? sender, EventArgs e)
        {
            ...
            if (centerOnNextResize && SelectedImage != null && ViewWidth > 1 && ViewHeight > 1)
            {
                centerOnNextResize = false;
        ...
        private void InitGL()
        {
            ...
            {
                Console.WriteLine($"BioImager GLFW Error {error}: {description}");
            });
        }
        ...
        private void InitializeOverviewOverlay()
        {
            ...
            overviewOverlay = new OverviewOverlayPanel
            {
                Name = "overviewOverlay",
                Location = new System.Drawing.Point(0, 0),
                Size = new System.Drawing.Size(overview.Width, overview.Height),
                Visible = false,
                Anchor = AnchorStyles.Top | AnchorStyles.Left,
                BackColor = System.Drawing.Color.Transparent,
            };
            overviewOverlay.Paint += OverviewOverlay_Paint;
            AttachViewerInput(overviewOverlay);
            viewpanel.Controls.Add(overviewOverlay);
            overviewOverlay.BringToFront();
        }

        private void BringRenderSurfacesToFront()
        {
            if (gl.Parent == viewpanel)
                gl.BringToFront();
            if (overviewOverlay != null && overviewOverlay.Parent == viewpanel)
                overviewOverlay.BringToFront();
        }
        ...
        private System.Drawing.Bitmap overviewPreviewBitmap;
        private bool centerOnNextResize;
        /// <summary>
        /// Called by SlideGLArea during the Skia rendering phase.
        /// Draws annotations on top of the GL-rendered tiles.
        /// </summary>
        private void Render(System.Drawing.Graphics canvas)
        {
            try
            {
                DrawOverlay(canvas);
                System.Drawing.Rectangle rf = new System.Drawing.Rectangle((int)canvas.Clip.GetBounds(canvas).X, (int)canvas.Clip.GetBounds(canvas).Y,
                   (int)canvas.Clip.GetBounds(canvas).Width, (int)canvas.Clip.GetBounds(canvas).Height);
                // Plugin rendering hook
                Plugins.Render(this, new PaintEventArgs(canvas, rf));
            }
            catch (Exception ex)
            {
        ...
        private void RefreshPyramidalOverlayTexture()
        {
            if (SelectedImage == null || !SelectedImage.isPyramidal || gl == null)
                return;
            if (gl.Width <= 0 || gl.Height <= 0)
                return;

            using System.Drawing.Bitmap overlay = new System.Drawing.Bitmap(gl.Width, gl.Height, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            using (System.Drawing.Graphics g = System.Drawing.Graphics.FromImage(overlay))
            {
                g.Clear(System.Drawing.Color.Transparent);
                Render(g);
            }

            gl.SetOverlayBitmap(overlay);
        }
        ...
        public void AddImage(BioImage im)
        {
            ...
            QueueInitialCenterAfterLayout(Images.Count - 1);
        }

        private void QueueInitialCenterAfterLayout(int imageIndex)
        {
            if (!IsHandleCreated)
                return;
            BeginInvoke(new System.Action(() =>
            {
                if (IsDisposed || !centerOnNextResize || imageIndex >= Images.Count)
                    return;
                centerOnNextResize = false;
        ...
        private async void InitPreview()
        {
            if (SelectedImage == null)
                return;
            overviewPreviewBitmap?.Dispose();
            overviewPreviewBitmap = null;
            if (SelectedImage.Resolutions.Count == 0)
            {
                ShowOverview = false;
                UpdateOverviewOverlay();
                return;
            ...
            int fullWidth = SelectedImage.Resolutions[0].SizeX;
            int fullHeight = SelectedImage.Resolutions[0].SizeY;
            double aspect = (double)fullWidth / Math.Max(1, fullHeight);
            int overviewWidth = aspect >= 1.0
                ? 120
                : (int)Math.Max(1, Math.Round(120 * aspect));
            int overviewHeight = aspect >= 1.0
                ? (int)Math.Max(1, Math.Round(120 / aspect))
                : 120;
            ...
            byte[] bytes;
            if (SelectedImage.Resolutions.Count == 1 && SelectedImage.Buffers.Count > 0)
            {
                using var sourceBitmap = ToSysBitmap(SelectedImage.Buffers[0]);
                overviewPreviewBitmap = CreateScaledOverviewPreview(sourceBitmap, overviewWidth, overviewHeight);
                ShowOverview = true;
                UpdateOverviewOverlay();
                return;
            }
            ...
            var sourceExtent = _openSlideBase?.Schema?.Extent ?? _slideBase?.Schema?.Extent;
            if (!sourceExtent.HasValue || (_openSlideBase == null && _slideBase == null))
                return;

            if (_openSlideBase != null)
            {
                int sourceLevel = _openSlideBase.Schema.Resolutions.Keys.Max();
                var sourceDim = _openSlideBase.SlideImage.GetLevelDimension(sourceLevel);
                int sourceWidth = (int)sourceDim.Width;
                int sourceHeight = (int)sourceDim.Height;
                bytes = await Task.Run(() => _openSlideBase.SlideImage.ReadRegion(sourceLevel, 0, 0, sourceWidth, sourceHeight));
                if (bytes == null)
                    return;
                using var sourceBitmap = CreateOverviewPreview32(bytes, sourceWidth, sourceHeight);
                overviewPreviewBitmap = CreateScaledOverviewPreview(sourceBitmap, overviewWidth, overviewHeight);
                ShowOverview = true;
                UpdateOverviewOverlay();
                return;
            }
            else if (_slideBase != null)
            {
                var slideSliceInfo = new BioLib.SliceInfo
                {
                    Extent = sourceExtent.Value,
                    Resolution = SelectedImage.GetUnitPerPixel(lev),
                    Coordinate = coord,
                    Parame = new BioLib.SliceParame
                    {
                        DstPixelWidth = overviewWidth,
                        DstPixelHeight = overviewHeight,
                        Quality = 100,
                    },
                };
                bytes = await Task.Run(() => _slideBase.GetSlice(slideSliceInfo, new PointD(0, 0), new AForge.Size(overviewWidth, overviewHeight)));
            }
            else
                return;
            if (bytes == null)
                return;
            overviewPreviewBitmap = CreateOverviewPreview(bytes, overviewWidth, overviewHeight);
            ShowOverview = true;
            UpdateOverviewOverlay();
            if (SelectedImage.isPyramidal)
                UpdateView();
        ...
        {
            get { if (SelectedImage.SlideBase == null) return true; else return false; }
        }
        ...
        public int ViewWidth
        {
            get
            {
                if (SelectedImage != null && !SelectedImage.isPyramidal && viewpanel != null && viewpanel.ClientSize.Width > 1)
                    return viewpanel.ClientSize.Width;
                return gl.Width;
            }
        }
        public int ViewHeight
        {
            get
            {
                if (SelectedImage != null && !SelectedImage.isPyramidal && viewpanel != null && viewpanel.ClientSize.Height > 1)
                    return viewpanel.ClientSize.Height;
                return gl.Height;
            }
        }
        ...
        public void UpdateSelectBoxSize(float size)
        {
            ...
            UpdateToolSelectBoxSize();
        }

        private const float MinSelectBoxScreenSize = 8f;
        private const float MinSelectionHitScreenSize = 8f;

        private float GetSelectBoxSize()
        {
            return GetSelectBoxSizeForScreenPixels(MinSelectBoxScreenSize);
        }

        private float GetSelectBoxSizeForScreenPixels(float minScreenSize)
        {
            if (SelectedImage != null && SelectedImage.isPyramidal)
            {
                float logicalSize = Math.Max(ROI.selectBoxSize, minScreenSize);
            ...
            double pixelSize = Math.Min(PxWmicron, PxHmicron);
            if (pixelSize <= 0 || double.IsNaN(pixelSize) || double.IsInfinity(pixelSize))
                pixelSize = 1;

            float screenSize = Math.Max(ROI.selectBoxSize, minScreenSize);
            return (float)(screenSize / (zoom * pixelSize));
        }

        private void UpdateToolSelectBoxSize()
        {
            Tools.selectBoxSize = GetSelectBoxSize();
        }
        ...
        public void UpdateOverlay()
        {
            ...
            if (SelectedImage != null && SelectedImage.isPyramidal)
            {
                RefreshPyramidalOverlayTexture();
                gl.Invalidate();
                overviewOverlay?.Invalidate();
            }
            else
        ...
        public void UpdateView(bool refreshImages = true)
        {
            UpdateStatus();
            update = true;
            UpdateToolSelectBoxSize();
        ...
        public void RenderFrame()
        {
            ...
            if (dBitmaps.Count != Images.Count)
                UpdateImages();
            if (HardwareAcceleration && sk != null)
            {
                Plugins.Render(this, sk);
                sk.BeginDraw();
                sk.RenderTarget2D.Clear(new RawColor4(1.0f, 1.0f, 1.0f, 1.0f));
                RectangleF rg = ToScreenRect(PointD.MinX, PointD.MinY, PointD.MaxX - PointD.MinX, PointD.MaxY - PointD.MinY);
                //dx.RenderTarget2D.Transform = SharpDX.Matrix3x2.Rotation((float)Math.PI);
                SharpDX.Direct2D1.SolidColorBrush pen = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(1.0f, 0.0f, 0.0f, 1.0f));
                SharpDX.Direct2D1.SolidColorBrush red = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(1.0f, 0.0f, 0.0f, 1.0f));
                SharpDX.Direct2D1.SolidColorBrush green = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(0.0f, 1.0f, 0.0f, 1.0f));
                SharpDX.Direct2D1.SolidColorBrush mag = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(1.0f, 0.0f, 1.0f, 1.0f));
                SharpDX.Direct2D1.SolidColorBrush blue = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(0.0f, 0.0f, 1.0f, 1.0f));
                SharpDX.Direct2D1.SolidColorBrush b = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(1.0f, 1.0f, 0.0f, 1.0f));
                SharpDX.Direct2D1.SolidColorBrush gray = new SharpDX.Direct2D1.SolidColorBrush(sk.RenderTarget2D, new RawColor4(0.5f, 0.5f, 0.5f, 1.0f));

                sk.RenderTarget2D.FillRectangle(ToSKRect(rg.X, rg.Y, rg.Width, rg.Height), gray);
                for (int x = 0; x < Images.Count; x++)
                {
                    if (!IsLayerEnabled(Images[x].PhysicalSizeX) && !Images[x].isPyramidal)
                        continue;
                    RectangleF r = ToScreenRect(Images[x].Volume.Location.X, Images[x].Volume.Location.Y, Images[x].Volume.Width, Images[x].Volume.Height);
                    ...
                    sk.RenderTarget2D.DrawBitmap(dBitmaps[x], ToSKRect(r.X, r.Y, r.Width, r.Height), 1.0f, SharpDX.Direct2D1.BitmapInterpolationMode.Linear);
                    if (x == selectedIndex)
                        sk.RenderTarget2D.DrawRectangle(ToSKRect(r.X, r.Y, r.Width, r.Height), blue);
                }
                ...
                foreach (BioImage bi in Images)
                {
                    ...
                    {
                        if (an.type == ROI.Type.Polygon || an.type == ROI.Type.Freeform || an.type == ROI.Type.Polyline)
                        {
                            if (zBar.Value != an.coord.Z || tBar.Value != an.coord.T)
                                continue;
                        }
                        ...
                        float width = (float)an.strokeWidth / w;
                        if (width <= 0)
                            width = 1;
                        sk.RenderTarget2D.StrokeWidth = width;
                        if (an.Selected)
                            b.Color = new RawColor4(1.0f, 0.0f, 1.0f, 1.0f);
                        else
                            b.Color = new RawColor4((float)an.strokeColor.R / 255f, (float)an.strokeColor.G / 255f, (float)an.strokeColor.B / 255f, 1.0f);
                        ...
                        if (an.type == ROI.Type.Line)
                        {
                            ...
                            sk.RenderTarget2D.DrawLine(new RawVector2((float)pf.X, (float)pf.Y), new RawVector2((float)pf2.X, (float)pf2.Y), b);
                            RectangleF[] rfs = ToScreenSpace(an.GetSelectBoxes(Tools.selectBoxSize));
                            for (int i = 0; i < rfs.Length; i++)
                            {
                                sk.RenderTarget2D.DrawRectangle(ToSKRect(rfs[i].X, rfs[i].Y, rfs[i].Width, rfs[i].Height), red);
                        ...
                        if (an.type == ROI.Type.Ellipse)
                        {
                            ...
                            RectangleF[] rfs = ToScreenSpace(an.GetSelectBoxes(Tools.selectBoxSize));
                            RawRectangleF rf = ToSKRect(rfs[0].X, rfs[0].Y, rfs[0].Width, rfs[0].Height);
                            ...
                            sk.RenderTarget2D.DrawEllipse(e, b);
                            for (int i = 0; i < rfs.Length; i++)
                            {
                                sk.RenderTarget2D.DrawRectangle(ToSKRect(rfs[i].X, rfs[i].Y, rfs[i].Width, rfs[i].Height), red);
                            }
                        ...
                        if (an.type == ROI.Type.Polygon && an.closed)
                        {
                            ...
                            for (int i = 0; i < an.Points.Count - 1; i++)
                            {
                                ...
                                sk.RenderTarget2D.DrawLine(pf, pf2, b);
                            }
                        ...
                        if (labels)
                        {
                            if (an.Text != null && an.type != ROI.Type.Label)
                            {
                                ...
                                RawRectangleF r = ToSKRect(fw, fh, s.Width, s.Height);
                                SharpDX.DirectWrite.TextFormat tex = new SharpDX.DirectWrite.TextFormat(sk.FactoryDWrite, an.family.ToString(), an.fontSize);
                        ...
                        if (an.Selected)
                        {
                            ...
                            if (rects.Count > 0)
                            {
                                for (int i = 0; i < rects.Count; i++)
                                {
                                    ...
                                    sk.RenderTarget2D.DrawRectangle(ToSKRect(ri.X, ri.Y, ri.Width, ri.Height), blue);
                                }
                            }
                            rects.Clear();
                        }
                ...
                if (ShowStage)
                {
                    ...
                    RectangleF vr = ToScreenRect(re.X, re.Y, re.W, re.H);
                    RawRectangleF rr = ToSKRect(vr.X, vr.Y, vr.Width, vr.Height);
                    sk.RenderTarget2D.DrawRectangle(rr, red);
                }

                if(Tools.currentTool!=null)
                if (Tools.currentTool.type == Tools.Tool.Type.rectSel || Tools.currentTool.type == Tools.Tool.Type.rect)// && Modifiers == ModifierType.Button1Mask)
                {
                    ...
                    sk.RenderTarget2D.DrawRectangle(ToSKRect((float)recd.X, (float)recd.Y, (float)recd.Width, (float)recd.Height), blue);
                }
                pen.Dispose();
                red.Dispose();
                mag.Dispose();
        ...
        */
        ...
        public void UpdateViewLegacy(bool refresh)
        {
            UpdateStatus();
            update = refresh;
            if (update)
        ...
        public async void UpdateImages()
        {
            ...
            */
            ...
            foreach (BioImage b in Images)
            {
                ...
                b.PyramidalSize = new AForge.Size(gl.Width, gl.Height);
                ...
                Bitmaps.Add(ToSysBitmap(bitmap));
                bi++;
            }
            update = true;
            UpdateRenderSurfaceMode();
        ...
        /// If the trackBarPanel is visible, hide it, set its height to 0, and set the gl height
        /// to 75. 
        /// If the trackBarPanel is not visible, show it, set the gl height to 75, and set the
        ...
        private void ImageView_MouseWheel(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            ...
            if (SelectedImage != null)
                if (SelectedImage.isPyramidal)
                {
                    if (Ctrl)
                    {
                        ZoomPyramidalAt(e.Location, e.Delta > 0 ? 0.95 : 1.05);
                        return;
                    }
                }
        ...
        private void ZoomPyramidalAt(System.Drawing.Point mouseLocation, double zoomFactor)
        {
            if (SelectedImage == null || !SelectedImage.isPyramidal)
                return;

            double oldResolution = Resolution;
            double oldOriginX = PyramidalOrigin.X;
            double oldOriginY = PyramidalOrigin.Y;
            double newResolution = oldResolution * zoomFactor;
            ...
            var schema = SelectedImage.OpenSlideBase != null
        ...
        public List<ROI> AnnotationsR
        {
            get
            {
                if (SelectedImage == null)
                    return new List<ROI>();
                return SelectedImage.Annotations.Where(an =>
                    an != null &&
                    an.coord.Z == SelectedImage.Coordinate.Z &&
                    an.coord.T == SelectedImage.Coordinate.T &&
                    (an.type == ROI.Type.Polygon || an.type == ROI.Type.Freeform || an.type == ROI.Type.Polyline || an.coord.C == RChannel.Index)).ToList();
            }
        }
        ...
        public List<ROI> AnnotationsG
        {
            get
            {
                ...
                return SelectedImage.Annotations.Where(an =>
                    ...
                    an.coord.Z == SelectedImage.Coordinate.Z &&
                    an.coord.T == SelectedImage.Coordinate.T &&
                    (an.type == ROI.Type.Polygon || an.type == ROI.Type.Freeform || an.type == ROI.Type.Polyline || an.coord.C == GChannel.Index)).ToList();
            }
        }
        ...
        public List<ROI> AnnotationsB
        {
            get
            {
                ...
                return SelectedImage.Annotations.Where(an =>
                    ...
                    an.coord.Z == SelectedImage.Coordinate.Z &&
                    an.coord.T == SelectedImage.Coordinate.T &&
                    (an.type == ROI.Type.Polygon || an.type == ROI.Type.Freeform || an.type == ROI.Type.Polyline || an.coord.C == BChannel.Index)).ToList();
            }
        }
        public List<ROI> AnnotationsRGB
        {
            get
            {
                ...
                {
                    ans.AddRange(SelectedImage.Annotations.Where(an =>
                        an != null &&
                        an.coord.Z == SelectedImage.Coordinate.Z &&
                        an.coord.T == SelectedImage.Coordinate.T &&
                        (an.type == ROI.Type.Polygon || an.type == ROI.Type.Freeform || an.type == ROI.Type.Polyline || an.coord.C == SelectedImage.Coordinate.C)));
                }
                return ans;
            }
        }
        ...
        private void DrawOverlay(System.Drawing.Graphics g)
        {
            ...
            foreach (BioImage bi in Images)
            {
                ...
                foreach (ROI an in bi.Annotations)
                {
                    ...
                    if (an.type == ROI.Type.Rectangle && an.BoundingBox.W > 0 && an.BoundingBox.H > 0)
                    {
                        RectangleD rec = ToScreenSpace(an.BoundingBox);
                        g.DrawRectangle(pen, new System.Drawing.RectangleF((float)rec.X, (float)rec.Y, (float)rec.W, (float)rec.H));
                        foreach (var item in ToScreenSpace(an.GetSelectBoxes(Tools.selectBoxSize)))
                        {
                            g.DrawRectangle(red, new System.Drawing.RectangleF((float)item.X, (float)item.Y, (float)item.W, (float)item.H));
                        }
                    ...
                    {
                        RectangleD rec = ToScreenSpace(an.BoundingBox);
                        g.DrawEllipse(pen, (float)rec.X, (float)rec.Y, (float)rec.W, (float)rec.H);
                        foreach (var item in ToScreenSpace(an.GetSelectBoxes(Tools.selectBoxSize)))
                        {
                            g.DrawRectangle(red, new System.Drawing.RectangleF((float)item.X, (float)item.Y, (float)item.W, (float)item.H));
                        }
                    ...
                    if (an.type == ROI.Type.Polygon && an.closed)
                    {
                        ...
                        {
                            System.Drawing.PointF[] pts = new System.Drawing.PointF[an.Points.Count];
                            for (int i = 0; i < an.Points.Count; i++)
                            {
                                var p = ToScreenSpace(an.Points[i].X, an.Points[i].Y);
                                pts[i] = new System.Drawing.PointF((float)p.X, (float)p.Y);
                            }
                            g.DrawLines(pen, pts);
                    ...
                    if (an.Selected)
                    {
                        ...
                        for (int i = 0; i < rects.Count; i++)
                        {
                            ...
                            g.DrawRectangle(blue, new System.Drawing.RectangleF((float)va[0].X, (float)va[0].Y, (float)va[0].W, (float)va[0].H));
                        }
            ...
            try
            {
                ...
                foreach (BioImage im in Images)
                {
                    if (!IsLayerEnabled(im.PhysicalSizeX) && !im.isPyramidal)
                        continue;
                    RectangleD r = ToScreenRect(im.Volume.Location.X, im.Volume.Location.Y, im.Volume.Width, im.Volume.Height);
                    double w = ToViewW(ViewWidth);
                    double h = ToViewH(ViewHeight);
                    ...
                    {
                        g.DrawImage(Bitmaps[i], (float)r.X, (float)r.Y, (float)r.W, (float)r.H);
                    }
                    if (i == SelectedIndex && !SelectedImage.isPyramidal)
                    {
                        rf[0] = new System.Drawing.RectangleF((float)r.X, (float)r.Y, (float)r.W, (float)r.H);
        ...
        private void UpdateOverviewOverlay()
        {
            ...
            overviewOverlay.Location = new System.Drawing.Point(0, 0);
            overviewOverlay.Size = new System.Drawing.Size(overview.Width, overview.Height);
            overviewOverlay.Visible = SelectedImage != null && ShowOverview && overviewPreviewBitmap != null;
            if (overviewOverlay.Visible)
            {
                overviewOverlay.BringToFront();
                overviewOverlay.Invalidate();
            }
        ...
        private void OverviewOverlay_Paint(object sender, PaintEventArgs e)
        {
            DrawOverview(e.Graphics);
        }

        private bool TryHandleOverviewInteraction(System.Drawing.Point location, MouseButtons buttons)
        {
            if (SelectedImage == null || !ShowOverview || overviewOverlay == null || overviewPreviewBitmap == null)
                return false;
            if ((buttons & MouseButtons.Left) != MouseButtons.Left)
                return false;
            if (!overviewOverlay.ClientRectangle.Contains(location))
                return false;

            int previewWidth = Math.Max(1, overviewOverlay.ClientSize.Width);
            int previewHeight = Math.Max(1, overviewOverlay.ClientSize.Height);
            if (SelectedImage.isPyramidal)
            {
                Resolution rs = SelectedImage.Resolutions[(int)Level];
                double dsx = OpenSlide
                    ? _openSlideBase.Schema.Resolutions[Level].UnitsPerPixel / Resolution
                    : _slideBase.Schema.Resolutions[Level].UnitsPerPixel / Resolution;

                double dx = ((double)location.X / previewWidth) * (rs.SizeX * dsx);
                double dy = ((double)location.Y / previewHeight) * (rs.SizeY * dsx);
                double w = (double)gl.Width / 2;
                double h = (double)gl.Height / 2;
                PyramidalOrigin = new PointD(dx - w, dy - h);
            }
            else
            {
                var volume = SelectedImage.Volume;
                if (volume.Width <= 0 || volume.Height <= 0)
                    return false;
                double dx = volume.Location.X + ((double)location.X / previewWidth) * volume.Width;
                double dy = volume.Location.Y + ((double)location.Y / previewHeight) * volume.Height;
                Origin = new PointD(-dx, -dy);
            }
            UpdateView();
            return true;
        }

        private void DrawOverview(System.Drawing.Graphics g)
        {
            if (SelectedImage == null || !ShowOverview || overviewPreviewBitmap == null)
                return;

            g.ResetTransform();
            g.DrawImage(overviewPreviewBitmap, new System.Drawing.Rectangle(0, 0, overview.Width, overview.Height));
            ...
            else
            {
                ...
                if (volume.Width > 0 && volume.Height > 0)
                {
                    double visibleWidth = Math.Abs(ToViewW(ViewWidth));
                    double visibleHeight = Math.Abs(ToViewH(ViewHeight));
                    double left = (-Origin.X) - (visibleWidth / 2.0);
                    double top = (-Origin.Y) - (visibleHeight / 2.0);
                    double scaleX = overview.Width / volume.Width;
                    double scaleY = overview.Height / volume.Height;
                    double dx = (left - volume.Location.X) * scaleX;
                    double dy = (top - volume.Location.Y) * scaleY;
                    double dw = visibleWidth * scaleX;
                    double dh = visibleHeight * scaleY;
                    g.DrawRectangle(Pens.Red, (int)dx, (int)dy, (int)dw, (int)dh);
                }
            }
            g.ResetTransform();
        ...
        private System.Drawing.Bitmap CreateOverviewPreview(byte[] rgbBytes, int width, int height)
        {
            ...
            try
            {
                ...
                unsafe
                {
                    fixed (byte* srcPtr = rgbBytes)
                    {
                        for (int y = 0; y < height; y++)
                        {
                            byte* srcRow = srcPtr + (y * srcStride);
                            byte* dstRow = (byte*)data.Scan0 + (y * dstStride);
        ...
        private System.Drawing.Bitmap CreateScaledOverviewPreview(System.Drawing.Bitmap sourceBitmap, int width, int height)
        {
            ...
            using (var g = System.Drawing.Graphics.FromImage(bitmap))
            {
                ...
                g.DrawImage(sourceBitmap, new System.Drawing.Rectangle(0, 0, width, height));
            }
            return bitmap;
        }

        private System.Drawing.Bitmap CreateOverviewPreview32(byte[] bgraBytes, int width, int height)
        {
            var bitmap = new System.Drawing.Bitmap(width, height, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            var rect = new System.Drawing.Rectangle(0, 0, width, height);
            var data = bitmap.LockBits(rect, System.Drawing.Imaging.ImageLockMode.WriteOnly, bitmap.PixelFormat);
            try
            {
                ...
                unsafe
                {
                    fixed (byte* srcPtr = bgraBytes)
                    {
                        for (int y = 0; y < height; y++)
                        {
                            byte* srcRow = srcPtr + (y * rowBytes);
                            byte* dstRow = (byte*)data.Scan0 + (y * dstStride);
        ...
        {
            return GetSelectionBoxHitSize();
        }

        public double GetSelectionBoxHitSize()
        {
            return GetSelectBoxSizeForScreenPixels(MinSelectionHitScreenSize);
        }
        ...
        private PointD GetStatusPoint(PointD p)
        {
            if (SelectedImage != null && SelectedImage.isPyramidal && Resolution != 0)
                return new PointD(p.X / Resolution, p.Y / Resolution);
            return p;
        }
        private void UpdateMouseStatus(PointD p)
        {
            if (SelectedImage == null)
                return;

            PointD displayPoint = GetStatusPoint(p);
            PointD samplePoint = SelectedImage.isPyramidal
                ? new PointD((p.X / Resolution) - PyramidalOrigin.X, (p.Y / Resolution) - PyramidalOrigin.Y)
                : p;

            mousePoint = "(" + (int)displayPoint.X + ", " + (int)displayPoint.Y + ")";
            mouseColor = "";

            AForge.Point s = SelectedImage.isPyramidal
                ? new AForge.Point(
                    Math.Max(1, SelectedImage.Resolutions[Math.Clamp(SelectedImage.Level, 0, SelectedImage.Resolutions.Count - 1)].SizeX),
                    Math.Max(1, SelectedImage.Resolutions[Math.Clamp(SelectedImage.Level, 0, SelectedImage.Resolutions.Count - 1)].SizeY))
                : new AForge.Point(SelectedImage.SizeX, SelectedImage.SizeY);
            if (samplePoint.X < 0 || samplePoint.Y < 0 || samplePoint.X >= s.X || samplePoint.Y >= s.Y)
                return;
            ...
            if (SelectedImage.isPyramidal &&
                ...
                SelectedImage.Resolutions[Math.Clamp(SelectedImage.Level, 0, SelectedImage.Resolutions.Count - 1)].PixelFormat == AForge.PixelFormat.Format16bppGrayScale)
            {
                try
                {
                    int frameIndex = SelectedImage.GetFrameIndex(zc, cc, tc);
                    int level = Math.Clamp(SelectedImage.Level, 0, SelectedImage.Resolutions.Count - 1);
                    int sx = (int)Math.Floor(samplePoint.X);
                    int sy = (int)Math.Floor(samplePoint.Y);
                    byte[] raw = SelectedImage.GetTileBytesRaw(frameIndex, level, sx, sy, 1, 1, SelectedImage.Coordinate)
                        .GetAwaiter()
                        .GetResult();
                    if (raw != null && raw.Length >= 2)
                    {
                        ushort value = (ushort)(raw[0] | (raw[1] << 8));
                        mouseColor = ", " + value;
                        return;
                    }
                }
                ...
                {
                    // Fall back to the generic buffer-backed path below.
                }
            }
            if (SelectedImage.isPyramidal)
            {
                try
                {
                    ...
                    int sx = (int)Math.Floor(samplePoint.X);
                    int sy = (int)Math.Floor(samplePoint.Y);
                    using Bitmap rawTile = SelectedImage.GetTile(frameIndex, level, sx, sy, 1, 1, SelectedImage.Coordinate, true)
                        .GetAwaiter()
                        .GetResult();
                    if (rawTile != null)
                    {
                        switch (rawTile.PixelFormat)
                        {
                            case AForge.PixelFormat.Format8bppIndexed:
                                mouseColor = ", " + (int)rawTile.GetValue(0, 0, 0);
                                return;
                            ...
                            case AForge.PixelFormat.Format32bppPArgb:
                                {
                                    int b = (int)rawTile.GetValue(0, 0, 0);
                                    int g = (int)rawTile.GetValue(0, 0, 1);
                                    int r = (int)rawTile.GetValue(0, 0, 2);
                                    mouseColor = ", " + r + "," + g + "," + b;
                                    return;
                                }
            ...
            {
                int r = (int)SelectedImage.GetValueRGB(zc, RChannel.Index, tc, (int)samplePoint.X, (int)samplePoint.Y, 0);
                int g = (int)SelectedImage.GetValueRGB(zc, GChannel.Index, tc, (int)samplePoint.X, (int)samplePoint.Y, 1);
                int b = (int)SelectedImage.GetValueRGB(zc, BChannel.Index, tc, (int)samplePoint.X, (int)samplePoint.Y, 2);
                mouseColor = ", " + r + "," + g + "," + b;
            }
            else
            {
                int r = (int)SelectedImage.GetValueRGB(zc, 0, tc, (int)samplePoint.X, (int)samplePoint.Y, 0);
                mouseColor = ", " + r;
            }
        }
        ...
        {
            if (TryHandleOverviewInteraction(e.Location, e.Button))
                return;
            ...
            {
                    PointD pencilIp = SelectedImage.ToImageSpace(p);
                    ...
                    g.FillEllipse(new Rectangle((int)pencilIp.X, (int)pencilIp.Y, (int)Tools.StrokeWidth, (int)Tools.StrokeWidth), pen.color);
                    UpdateImage();
                }
        ...
        private void pictureBox_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (Tools.currentTool.type == Tools.Tool.Type.move && e.Button == MouseButtons.Left)
            {
                float width = (float)GetSelectionBoxHitSize();
                float height = (float)GetSelectionBoxHitSize();
                foreach (BioImage bi in Images)
                {
                    foreach (ROI an in bi.Annotations)
                    {
                        if (an.GetSelectBound(width, height).IntersectsWith(new RectangleD(p.X, p.Y, 1, 1)))
                        {
                            ...
                            an.Selected = true;
                            RectangleD[] sels = an.GetSelectBoxes(Tools.selectBoxSize);
                            double hitSize = GetSelectionBoxHitSize();
                            RectangleD r = new RectangleD(p.X - hitSize / 2, p.Y - hitSize / 2, hitSize, hitSize);
        ...
        {
            string[] sts = System.Windows.Forms.Clipboard.GetText().Split(BioImage.NewLine);
        ...
        public RectangleD ToScreenRect(double x, double y, double w, double h)
        {
            if (SelectedImage == null)
            {
                ...
                return new RectangleD(pf.X, pf.Y, ToScreenScaleW(w), ToScreenScaleH(h));
            }
            if (SelectedImage.isPyramidal)
            {
                PointD d = ToScreenSpace(x, y);
        ...
        /// @param h height of the rectangle
        /// 
        /// @return A SKRect object.
        public SKRect ToSKRect(double x, double y, double w, double h)
        {
            double xd = gl.Width / 2;
            double yd = gl.Height / 2;
            return new SKRect((float)(xd - x), (float)(yd - y), (float)(xd - (x + w)), (float)(yd - (y + h)));
        }
        ...
        public RectangleF[] ToScreenSpace(RectangleF[] p)
        {
            ...
            for (int i = 0; i < p.Length; i++)
            {
                rs[i] = ToScreenSpace(p[i]).ToRectangleF();
            }
            return rs;
        }
        ...
        private void ImageView_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
        ...
        public void GoToImage(int i)
        {
            if (Images.Count <= i)
                return;
            SelectedIndex = i;
            BioImage image = Images[i];
            int viewportWidth = GetEffectiveViewWidth();
            int viewportHeight = GetEffectiveViewHeight();
            if (image.Type == BioImage.ImageType.pyramidal || image.isPyramidal)
            {
                CenterPyramidalImage(image, viewportWidth, viewportHeight);
                UpdateView();
                return;
            }

            double dx = image.Volume.Width / 2;
            double dy = image.Volume.Height / 2;
            origin = new PointD(-(image.Volume.Location.X + dx), -(image.Volume.Location.Y + dy));
            double screenWidth = Math.Abs(ToScreenScaleW(image.Volume.Width));
            double screenHeight = Math.Abs(ToScreenScaleH(image.Volume.Height));
            double wx = screenWidth > 0 ? viewportWidth / screenWidth : 1;
            double wy = screenHeight > 0 ? viewportHeight / screenHeight : 1;
            double fitScale = Math.Min(wx, wy);
            if (fitScale <= 0 || double.IsNaN(fitScale) || double.IsInfinity(fitScale))
                fitScale = 1;
            scale = new SizeF((float)fitScale, (float)fitScale);
            update = true;
            UpdateView();
        }

        private int GetEffectiveViewWidth()
        {
            int width = ViewWidth;
            if (width <= 1)
                width = viewpanel?.ClientSize.Width ?? Width;
            if (width <= 1)
                width = Width;
        ...
        private int GetEffectiveViewHeight()
        {
            ...
            if (height <= 1)
                height = viewpanel?.ClientSize.Height ?? Height;
            if (height <= 1)
                height = Height;
            return Math.Max(1, height);
        }

        private void CenterPyramidalImage(BioImage image, int viewportWidth, int viewportHeight)
        {
            if (!TryGetPyramidalWorldBounds(image, out double minX, out double minY, out double maxX, out double maxY))
                return;
            ...
            double fitResolution = Math.Max(worldWidth / viewportWidth, worldHeight / viewportHeight) * 1.02;
            if (fitResolution <= 0 || double.IsNaN(fitResolution) || double.IsInfinity(fitResolution))
                fitResolution = image.Resolution > 0 ? image.Resolution : 1;

            image.Resolution = fitResolution;
            double viewportWorldWidth = viewportWidth * fitResolution;
            double viewportWorldHeight = viewportHeight * fitResolution;
            double centerX = minX + (worldWidth / 2);
            double centerY = minY + (worldHeight / 2);

            image.PyramidalOrigin = new PointD(
                centerX - (viewportWorldWidth / 2),
                -(centerY + (viewportWorldHeight / 2)));
        }

        private bool TryGetPyramidalWorldBounds(BioImage image, out double minX, out double minY, out double maxX, out double maxY)
        {
            minX = minY = maxX = maxY = 0;
            BruTile.ITileSchema schema = null;
            if (image.OpenSlideBase != null)
                schema = image.OpenSlideBase.Schema;
            else if (image.SlideBase != null)
                schema = image.SlideBase.Schema;

            if (schema != null)
            {
                minX = schema.Extent.MinX;
                minY = schema.Extent.MinY;
                maxX = schema.Extent.MaxX;
                maxY = schema.Extent.MaxY;
            ...
            double unitsPerPixel = image.GetUnitPerPixel(0);
            if (unitsPerPixel <= 0 || double.IsNaN(unitsPerPixel) || double.IsInfinity(unitsPerPixel))
                unitsPerPixel = 1;

            minX = 0;
            maxX = image.Resolutions[0].SizeX * unitsPerPixel;
            maxY = 0;
            minY = -(image.Resolutions[0].SizeY * unitsPerPixel);
            return maxX > minX && maxY > minY;
        }
        ...
        private void Initialize(string file, BioImage bm)
        {
            ...
            if (bm.OpenSlideBase == null)
            {
                ...
                slideRenderer = new SlideRenderer(gl);
                slideRenderer.SetSource(_slideBase);
            }
            else
            {
                _openSlideBase = bm.OpenSlideBase;
                slideRenderer = new SlideRenderer(gl);
                slideRenderer.SetSource(_openSlideBase);
            }
        }
        ...
        private sealed class OverviewOverlayPanel : Panel
        {
            public OverviewOverlayPanel()
            {
            ...
            {
                // Paint only the overview content to avoid flicker over the GL surface.
            }
        }

        private sealed class BufferedViewPanel : Panel
        {
            public BufferedViewPanel()
            {
                SetStyle(ControlStyles.UserPaint |
                         ControlStyles.AllPaintingInWmPaint |
                         ControlStyles.OptimizedDoubleBuffer |
