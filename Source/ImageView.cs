...
namespace BioImager
{
    ...
    public partial class ImageView : UserControl, IDisposable
    {
        ...
        public PointD PyramidalOrigin
        {
            ...
            set
            {
                if (SelectedImage.Resolutions[Level].SizeX > value.X || SelectedImage.Resolutions[Level].SizeY > value.Y)
                {
        ...
        private void setTextSelectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (ROI item in AnnotationsRGB)
            {
                if (item.selected)
                {
                    ...
                    item.strokeColor = System.Drawing.Color.FromArgb(input.color.R, input.color.G, input.color.B);
                    UpdateOverlay();
                }
            }
        }
